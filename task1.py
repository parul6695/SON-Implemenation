...
case = 2
supp_threshold = 9
input_file_path = r"D:\DM sem2\Assignements\A2 Data\small2.csv" #sys.argv[3]
output_file_path = r"D:\DM sem2\Assignements\A2 Data\output_task1_case2"#sys.argv[4]
'''
...
#forming user's id as a key for case1 and businesses as a key for case2
#case=1
if case == 1:
    #Eliminating duplicates using mapValues if 1 user has same business_id twice
    finalRdd = finalRdd.map(lambda e: (e[0], e[1])).groupByKey().mapValues(lambda e: set(e))
elif case==2 :
    finalRdd = finalRdd.map(lambda e: (e[1], e[0])).groupByKey().mapValues(lambda e: set(e))
